...
- `src/note-interchange.ts` owns pure portable Markdown note copy/export formatting, frontmatter metadata parsing, leading-H1 import handling, and safe `.md` filenames.
- `src/note-import-files.ts` owns client-side TXT/Markdown/DOCX/PDF/VTT/SRT file validation, text-to-import parsing, Markdown interop handoff, and audio file extension/size summary handling for the Notes import panel; it does not parse or durably persist raw audio.
...
- `tests/note-interchange.test.ts` checks portable Markdown copy formatting, frontmatter export/import round trips, generic leading-H1 Markdown import, metadata preservation, access-scope parsing, and safe filenames.
...
- Build passes.
- 265/265 default tests pass; the opt-in live Supabase RLS test skips when Docker/local Supabase is unavailable.
...
- Note capture is typed/pasted text plus TXT/Markdown/DOCX/PDF/VTT/SRT file-content import, single-note Markdown copy/export, and consent-gated audio transcription import, with imports applying parsed content to the unsaved workbench, explicit save for existing notes, and server-backed draft recovery for unsaved workbench content. Markdown interop v1 is portable-file only: exported notes include simple frontmatter, and imported team scope preserves the category without trying to map teams across workspaces. Audio transcription v1 uses an injectable provider seam and an opt-in HTTP adapter, but no default vendor is configured; no raw audio is durably stored, and only job metadata plus applied transcript chunks persist. Transcript-derived claims now carry chunk citations when note text came from applied chunks. Bulk note export, direct Notion/Evernote/OneNote APIs, durable file storage, scanned-PDF OCR, vendor-specific transcription selection, and RMS integrations remain deferred.
